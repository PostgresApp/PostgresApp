...
class ServerManager: NSObject {
	...
	func createDefaultServer() {
        ...
        if autocreatedVersions.contains(version) {
            return
        }
        ...
        for server in servers {
            if server.binPath.contains("/\(version)/") {
                return
            }
        }
        ...
        servers.insert(Server(name: "PostgreSQL \(version)", version: version, startOnLogin: true), at: 0)
        saveServers()
        
        autocreatedVersions.append(version)
